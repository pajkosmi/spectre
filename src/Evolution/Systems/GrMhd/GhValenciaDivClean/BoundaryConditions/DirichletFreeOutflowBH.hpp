 ...
 * variables, used for after a BH forms.  This leads to setting the slope of the
 * spacetime variables to 0 at the inner BH boundary.
 */
template <typename System>
class DirichletFreeOutflowBH final : public BoundaryCondition {
 private:
  template <typename T>
 ...
 public:
  ...
  static constexpr Options::String help{
      "DirichletFreeOutflowBH boundary conditions using either analytic "
      "solution "
      "or analytic data for GH variables and hydro free outflow for GRMHD"};

  DirichletFreeOutflowBH() = default;
  DirichletFreeOutflowBH(DirichletFreeOutflowBH&&) = default;
  DirichletFreeOutflowBH& operator=(DirichletFreeOutflowBH&&) = default;
  DirichletFreeOutflowBH(const DirichletFreeOutflowBH&);
  DirichletFreeOutflowBH& operator=(const DirichletFreeOutflowBH&);
  ~DirichletFreeOutflowBH() override = default;

  explicit DirichletFreeOutflowBH(CkMigrateMessage* msg);

  explicit DirichletFreeOutflowBH(
      std::unique_ptr<evolution::initial_data::InitialData>
          analytic_prescription);

  WRAPPED_PUPable_decl_base_template(
      domain::BoundaryConditions::BoundaryCondition, DirichletFreeOutflowBH);
