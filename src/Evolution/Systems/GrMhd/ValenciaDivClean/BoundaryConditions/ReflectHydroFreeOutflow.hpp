...
/*!
 * \brief Apply reflective inner boundary conditions for velocity and free
 * boundary condition at the outer boundary for GRMHD primitive variables.
 *
 * All primitive variables at the boundary are copied into ghost zone except :
 *
 *  - Velocity has an opposite sign in the guard zones adjacent to the origin
 */
class ReflectHydroFreeOutflow final : public BoundaryCondition {
 private:
 ...
 public:
  ...
  ReflectHydroFreeOutflow() = default;
  ReflectHydroFreeOutflow(ReflectHydroFreeOutflow&&) = default;
  ReflectHydroFreeOutflow& operator=(ReflectHydroFreeOutflow&&) = default;
  ReflectHydroFreeOutflow(const ReflectHydroFreeOutflow&) = default;
  ReflectHydroFreeOutflow& operator=(const ReflectHydroFreeOutflow&) = default;
  ~ReflectHydroFreeOutflow() override = default;

  explicit ReflectHydroFreeOutflow(CkMigrateMessage* msg);

  WRAPPED_PUPable_decl_base_template(
      domain::BoundaryConditions::BoundaryCondition, ReflectHydroFreeOutflow);
